          ...
          {[...Array(totalTasks)].map((_, index) => {
            ...
            return (
              <React.Fragment key={index}>
                {showLine && (
                  <div className={`h-1.5 w-6 ${isCompleted ? 'bg-custom-blue dark:bg-custom-yellow' : 'bg-custom-dark-grey dark:bg-custom-dark-grey'}`} />
                )}
                <div
                  className={`h-6 w-6 rounded-full ${isCompleted ? 'bg-custom-blue dark:bg-custom-yellow' : 'bg-custom-dark-grey dark:bg-custom-dark-grey'}`}
                />
              </React.Fragment>
            );
          })}
